...
unsigned  long long         array_block_size;             //连续空间块配置大小
unsigned  long long         link_block_size;              //冲突空间块配置大小
...
unsigned  long long     record_num       ;/*插入数据原表的块的行号*/
int (*hash_fun)(unsigned  long long ,/*返回哈希表的行号*/struct  mem_block_t *,unsigned  long long *);
...
long                    block_no         ;/*插入数据原表所在的块号*/
unsigned  long long     record_num       ;/*插入数据原表的块的行号*/
int (*hash_fun)(char * key,/*连续空间块*/struct  mem_block_t * array_block,/*返回哈希表的行号*/unsigned  long long * record_num);
...
inline int integer_hash_fun(unsigned  long long key,/*连续空间块*/struct  mem_block_t * array_block,/*返回哈希表的行号*/unsigned  long long * record_num)
{
	...
	unsigned long long num = hash64shift(key)%( array_block->space_size / MEM_HASH_ENTRY_SIZE ) ;
	*record_num            =  num;
...
inline int str_hash_fun(char * key,/*连续空间块*/struct  mem_block_t * array_block,/*返回哈希表的行号*/unsigned  long long * record_num)
{
	...
	//先平均分布，再取余数	
	unsigned long long num = BKDRHash(key) % ( array_block->space_size / MEM_HASH_ENTRY_SIZE ) ;
	*record_num            =  num;
...
inline int mem_hash_index_insert_long(
                        ...
                        /* in */unsigned  long long     record_num       ,/*插入数据原表的块的行号*/
                        /* in */int (*hash_fun)(unsigned  long long ,/*返回哈希表的行号*/struct  mem_block_t *,unsigned  long long *),
                        /* out */struct    record_t   **  out_record_ptr,
