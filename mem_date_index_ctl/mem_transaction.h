...
unsigned  long long         		block_no;           //块号					 --- 需要在业务中填充 用于重做
unsigned  long long         		record_num;			    //行号           --- 需要在业务中填充 用于重做
...
inline int extend_transaction_manager()
{
  ...
  free(old);
  mem_transaction_t * trans;
  unsigned  long long i = transaction_manager.transaction_table_num/2;
	for(;i<transaction_manager.transaction_table_num;++i)
	{
		trans = &(transaction_manager.transaction_tables[i]);
		trans->is_used = 0;
...
unsigned  long long       block_no			= item->block_no;           //块号					 --- 需要在业务中填充 用于重做
unsigned  long long       record_num		= item->record_num;			    //行号           --- 需要在业务中填充 用于重做
