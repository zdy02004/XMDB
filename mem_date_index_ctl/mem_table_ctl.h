...
{
unsigned  long long     hash_lkey;						  //整型 hash   key 值
long                    block_no;			          //存数据所在的块号
unsigned  long long     record_num; 					  //存数据所在的行号
long                    link_block_no;          //冲突存储区块号
unsigned  long long     link_record_num;        //冲突数据在冲突块的行号
...
{
unsigned  long long     rbtree_lkey;						  //整型 rbtree   key 值
long                    block_no;			          //存数据所在表的块顺序号
unsigned  long long     record_num; 					  //存数据所在的行号
long                    parent_block_no;			  //父节点索引所在的块号
unsigned  long long     parent_record_num; 			//父节点索引所在表的块顺序号
long                    left_block_no;			    //左节点索引所在的块号
unsigned  long long     left_record_num; 				//左节点索引所在的行号
long                    right_block_no;			    //右节点索引所表的块顺序号
unsigned  long long     right_record_num; 			//右节点索引据所在的行号
color_t                 color;
...
long long                   last_free_pos;      // 用于维护空闲链表的位置信息
unsigned long long          next_free_pos;      // 用于维护空闲链表的位置信息
...
inline int get_block_by_record(struct mem_table_t *mem_table ,struct record_t * record_ptr,struct  mem_block_t ** mem_block)
{
  ...
  unsigned  long long   record_num = record_ptr->record_num;
...
inline int  get_record(mem_table_t *mem_table,long block_no,long long record_num,struct record_t ** record_ptr) 
{  																																																	
  int err;
  int i = 0;																																													
