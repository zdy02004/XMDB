...
UpdateAnalyser(rapidjson::Value & value ,rapidjson::Document & doc_  ):QueryAnalyser( value , doc_ ),set_lists(NULL)
{
	oper_type = BASIC_OPS_UPDATE ;
	 //检查是否有 set_list                                        
	check_and_assign_member(where_list,"SET_CLAUSE");   
	check_and_assign_member(from_list,"TABLE_NAME");
	
	if( (*from_list).HasMember("TABLE_NAME") && (*from_list)["TABLE_NAME"].HasMember("str_value_") )
		{
...
DEBUG("resolve_update_list_help -------- { \n");
				...
				rapidjson_log(v);
  				...
  				{
  					const_value = string((*from_list)["const_type"]["str_value_"].GetString()); 	// 值
  					if( (*v)["EXPR"].HasMember("const_type")    )  
  					{
  						const_type = string((*from_list)["const_type"]["str_value_"].GetString()); 	// 值类型   	
  					}  				 
  				}  		
  				set_value sv;
