...
// 不建议从中分配大于 MEM_POOL_MAX_SIZE 65536 的内存,
// 如果内存池内的内存，驻留时间过长，过大，应该适当增大 BLOCK_NUM
// 不宜总量超过20G

#define BLOCK_NUM (1024*8) // 该产数决定了内存池的初始大小 684211* BLOCK_NUM/1024/1024 M ，（1024*32）大约为
#define MAX_TIMES 1024*128  // 注意是2的n次方
// 9215 8191
...
//mem_pool_print(my_mem_pool);
printf("mem_pool have created! \n");
int k=0;
unsigned int sum;
for(;k<MEM_POOL_SLOT_SIZE;++k)
{
	sum += mem_pool_size[k];
}
//sum *= MAX_TIMES;
