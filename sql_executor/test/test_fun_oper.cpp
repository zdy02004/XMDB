...
/*
 ...
 g++ -g test_fun_oper.cpp ../../SQL/sql_parser/*.o  -std=gnu++11 -o main -w  -lpthread 
...
const char *sql_str = ""
 ...
 //  "select  user_name,  qty  as quantity from  table_2 t where t.user_name = 'c' and  t.ff is null and t.ee like '%cc'";
      "select  t.user_name,  qty  as quantity,sum(e+f+1),f*2,t3.user_id + t4.user_id,max(1,2) from  table_2 t,table_3 t3 where t.user_name = t3.user_name ";
...
//优化前
 ...
 for (auto& v : result.Doc["ROOT"]["children"].GetArray() ){
    ...
    resolve_to_opper_node(   &qa ,  projection_fun_oper_lists , projection_meta	);
		...
		record_typle output_tuple;
		input_tuple.meta = input_meta ;// 单表用单表，多表用join
		output_tuple.meta = projection_meta ;

			
		for(auto &row : ret_list ){			
			generic_result result;
			result.allocat( projection_meta.size() );
			
			input_tuple.result = &vv ;
